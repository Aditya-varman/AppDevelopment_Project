...
import plansData from './Hplandata.json'; // Import JSON data
...
const Hplan = () => {
  ...
  const [filteredPlans, setFilteredPlans] = useState(plansData); // Initialize with JSON data
  const [loading, setLoading] = useState(false); // No need to fetch, so set to false
    ...
    if (selectedPlan) {
      const handleBuy = (title) => {
        var options = {
          ...
          description: `For testing purpose - Plan: ${title}`,
      ...
      const updateSelectedPlan = (userId, selectedPlan) => {
        fetch(`http://localhost:8080/register/updatePlan/${userId}`, {
          method: "PUT",
          headers: {
          ...
          .then((data) => {
            console.log("Plan update response:", data);
          })
          .catch((error) => {
      ...
      updateSelectedPlan(userId,selectedPlan);
